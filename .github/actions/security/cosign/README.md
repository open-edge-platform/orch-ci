...
A GitHub Action to **sign container images or binaries using [Cosign](https://docs.sigstore.dev/quickstart/quickstart-cosign/)** from [Sigstore](https://github.com/sigstore/cosign). This action automates **signing, verification, and artifact handling** in CI/CD workflows.
...
- Downloads and uploads Cosign artifacts (signature, certificate, bundle, attestation) as GitHub artifacts.
...
- [GitHub token](https://docs.github.com/en/actions/security-guides/automatic-token-authentication) with permissions for container registry login.
- Docker installed if signing container images.
- Cosign is automatically installed by this action (no manual installation required).
...
| `artifact`   | Full image reference (e.g., `ghcr.io/org/app:tag`) or binary path           | ✅       | —            |
...
All signatures, certificates, bundles, and attestations are uploaded as GitHub Actions artifacts with a unique suffix, e.g., `cosign-image-3f2a1b`.
...
2. **Login (Images only)** — Authenticates to GitHub Container Registry using `gh_token`.
3. **Signing**:
   - **Image**: Keyless signing directly to the registry.
   - **Binary**: Local signing with output files (`sig`, `cert`, `bundle`).
4. **Fetch Artifacts** (Images only) — Downloads signature, certificate, and attestation.
5. **Verify Signature** — Ensures authenticity using OIDC-issued certificates.
6. **Upload Artifacts** — All Cosign artifacts are uploaded to GitHub Actions for traceability.
...
> 🔒 **Security Note:** Keyless signing uses ephemeral OIDC tokens. No long-lived secrets are required in your repository.
